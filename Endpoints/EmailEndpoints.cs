...
public static class EmailEndpoints
{
    ...
    {
        const string ModelName = nameof(Email);
        var tag = new List<OpenApiTag> { new() { Name = ModelName } };

        app.MapGet("/api/emails/{id:long}",
        async (long id, [FromServices] IEmailService service) =>
        {
            var entitie = await service.GetByIdAsync(id);
            if (entitie is null) return Results.NoContent();
            return Results.Ok(entitie);
        })
        .Produces((int)HttpStatusCode.OK, typeof(Email))
        ...
        async ([AsParameters] EmailFilters filters, [FromServices] IEmailService service) =>
        {
            var entities = await service.GetAllAsync(filters);
            if (entities.Count == 0) return Results.NoContent();
            return Results.Ok(entities);
        })
        .Produces((int)HttpStatusCode.OK, typeof(List<Email>))
            ...
            [FromServices] INotificationService notification) =>
        {
            ...
            return Results.Created($"/emails/{entitie!.Id}", entitie);
        })
        ...
        app.MapDelete("/api/emails/{id:long}",
        async (
            long id,
            [FromServices] IEmailService service,
            [FromServices] INotificationService notification) =>
