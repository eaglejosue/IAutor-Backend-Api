...
{
    public static void MapParamEndpoints(this WebApplication app)
    {
        const string ModelName = nameof(Param);
        var tag = new List<OpenApiTag> { new() { Name = ModelName } };

        app.MapGet("/api/params/{key}",
        async (string key, [FromServices] IParamService service) =>
        {
            var entitie = await service.GetByKeyAsync(key);
            if (entitie is null) return Results.NoContent();
            return Results.Ok(entitie);
        })
        .Produces((int)HttpStatusCode.OK, typeof(Param))
        .WithName($"{ModelName}ByKey")
        ...
        async ([AsParameters] ParamFilters filters, [FromServices] IParamService service) =>
        {
            var entities = await service.GetAllAsync(filters);
            if (entities.Count == 0) return Results.NoContent();
            return Results.Ok(entities);
        })
        .Produces((int)HttpStatusCode.OK, typeof(List<Param>))
        ...
        app.MapPost("/api/params",
        async (
            Param model,
            [FromServices] IParamService service,
            [FromServices] INotificationService notification) =>
        {
            ...
            return Results.Created($"/params/{entitie!.Id}", entitie);
        })
