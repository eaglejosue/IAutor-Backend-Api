...
public static class ChapterEndpoint
{
    public static void MapChapterEndpoints(this WebApplication app)
    {
        ...
        app.MapGet("/api/chapters/{id:long}",
        async (long id, [FromServices] IChapterService service) =>
        {
            var entitie = await service.GetByIdAsync(id);
            if (entitie is null) return Results.NoContent();
            return Results.Ok(entitie);
        })
        .Produces((int)HttpStatusCode.OK, typeof(Chapter))
        ...
        async (
            [AsParameters] ChapterFilters filters,
        ...
        .Produces((int)HttpStatusCode.OK, typeof(List<Chapter>))
